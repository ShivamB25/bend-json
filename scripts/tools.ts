...
function checkout(path: string): string | null {
  const inspect = (args: readonly string[]): SpawnSyncReturns<string> => {
...
export const ENV: NodeJS.ProcessEnv = { ...process.env, BEND_NO_TELEMETRY: '1' };
...
export function check(file: string): SpawnSyncReturns<string> {

...
import {limits,Null,BooleanValue,NumberValue,Text,ArrayValue,ObjectValue,assertAst,done,failure,encodeExpected,checkText,strictDecode,codepoints} from './support.mjs';
const N=NumberValue,T=Text,A=ArrayValue,O=ObjectValue,B=BooleanValue;
const ok=(id,text,value,limit=limits())=>({id:`directed/${id}`,text,limits:limit,expect:{kind:'done',value,encoded:encodeExpected(value)}});
const bad=(id,text,code,offset,limit=limits())=>({id:`directed/${id}`,text,limits:limit,expect:{kind:'fail',code,offset}});
...
export function* textCases() {
  yield ok('slice','{"x":[-1.5e+2,"\\u0041"]}',O([['x',A([N('-1.5e+2'),T('A')])]]));
  for(const [id,text,value] of [['null','null',Null()],['true','true',B(true)],['false','false',B(false)],['zero','0',N('0')],['empty-text','""',T('')],['text','"x"',T('x')],['empty-array','[]',A([])],['mixed-array','[null,true,"x",0]',A([Null(),B(true),T('x'),N('0')])],['empty-object','{}',O([])],['object','{"a":1}',O([['a',N('1')]])],['nested','[{"a":[{},[],false]},[]]',A([O([['a',A([O([]),A([]),B(false)])]]),A([])])]])yield ok(`root/${id}`,text,value);
  for(const [text,code,offset] of [['','PUnexpectedEnd',0],[' \t\r\n','PUnexpectedEnd',4],['Null','PUnexpectedCharacter',0],['TRUE','PUnexpectedCharacter',0],['nul','PUnexpectedEnd',3],['tru','PUnexpectedEnd',3],['fals','PUnexpectedEnd',4],['nux','PUnexpectedCharacter',2],['falsex','PTrailingContent',5]])yield bad(`literal/${JSON.stringify(text)}`,text,code,offset);
  yield ok('whitespace/all',' \t\r\n{ \t"a"\r:\n[\ttrue \r,\nnull\t]\r}\n',O([['a',A([B(true),Null()])]]));
  for(const ch of ['\v','\f','\u00a0','\u2060']){yield bad(`whitespace/start/${ch.codePointAt(0)}`,ch+'null','PUnexpectedCharacter',0);yield bad(`whitespace/suffix/${ch.codePointAt(0)}`,'null'+ch,'PTrailingContent',4);}
  ...
  const parseNumberBad=[['01',1],['-',1],['1.',2],['1e',2],['1e+',3],['1e-',3],['-Infinity',1],['--1',1],['1+2',1],['1..2',2]];
  ...
  const escaped=[['"\\u0041"','A'],['"\\\"\\\\\\/\\b\\f\\n\\r\\t"','"\\/\b\f\n\r\t'],['"\\u0000"','\0'],['"A\\u0000B"','A\0B']];
  for(let i=0;i<escaped.length;i++)yield ok(`escapes/${i}`,...escaped[i].map((x,j)=>j===1?T(x):x));
  ...
  for(const value of ['é','e\u0301','中文','😀','\u007f','\ufdd0','\ufffe','\uffff','\u{1fffe}','\u{10fffe}','\u{10ffff}','\u2028\u2029','�'])yield ok(`unicode/${[...value].map(c=>c.codePointAt(0).toString(16)).join('-')}`,'"'+value+'"',T(value));
  for(const [text,value] of [['"\\uD834\\uDD1E"','𝄞'],['"\\uD800\\uDC00"','\u{10000}'],['"\\uDBFF\\uDFFF"','\u{10ffff}']])yield ok(`surrogate/${text}`,text,T(value));
  ...
  const containers=[['[1,]','PUnexpectedCharacter',3],['[,1]','PUnexpectedCharacter',1],['[1,,2]','PUnexpectedCharacter',3],['[1 2]','PExpectedCommaOrEnd',3],['[1','PUnexpectedEnd',2],['[}','PUnexpectedCharacter',1],['{"a":1,}','PUnexpectedCharacter',7],['{"a" 1}','PExpectedColon',5],['{"a":1 "b":2}','PExpectedCommaOrEnd',7],['{"a":}','PUnexpectedCharacter',5],['{a:1}','PUnexpectedCharacter',1],['{"a":1]','PExpectedCommaOrEnd',6],['{]','PUnexpectedCharacter',1],['[truex]','PExpectedCommaOrEnd',5]];
  ...
  for(const [text,offset] of [['null true',5],['{}[]',2],['1x',1],['"x"garbage',3],['null\0',4],['"é"x',3],['"😀"x',3]])yield bad(`suffix/${JSON.stringify(text)}`,text,'PTrailingContent',offset);
  ...
  for(const n of [1,2,3]){let value=Null();for(let j=0;j<n;j++)value=A([value]);const text=encodeExpected(value);yield n<=2?ok(`limits/depth/${n}`,text,value,limits({max_depth:2n})):bad(`limits/depth/${n}`,text,'PDepthLimit',2,limits({max_depth:2n}));}
  ...
  for(const field of Object.keys(limits()).filter(k=>k!=='$')){yield bad(`limits/invalid/${field}`,'null','PInvalidLimits',0,limits({[field]:16777217n}));yield ok(`limits/cap/${field}`,'null',Null(),limits({[field]:16777216n}));}
  ...
  const nested='['.repeat(128)+'0'+']'.repeat(128);let nv=N('0');for(let i=0;i<128;i++)nv=A([nv]);yield ok('large/depth-default',nested,nv);yield bad('large/depth-over','['.repeat(129)+'0'+']'.repeat(129),'PDepthLimit',128);
}
export function* numberCases() {
  ...
  for(const [id,text,overrides,code,offset] of [['zero-input','1',{max_input:0n},'PInputLimit',0],['input','123',{max_input:2n},'PInputLimit',2],['number','123',{max_number:2n},'PNumberLimit',2],['values','1',{max_values:0n},'PValueLimit',0],['scalar','\ud800',{},'PInvalidScalar',0]])yield {id:`number/resource/${id}`,text,limits:limits(overrides),expect:{kind:'fail',code,offset}};
  ...
  for(const field of Object.keys(limits()).filter(k=>k!=='$'))yield {id:`number/invalid-limit/${field}`,text:'0',limits:limits({[field]:16777217n}),expect:{kind:'fail',code:'PInvalidLimits',offset:0}};
}
export function* encoderCases() {
  ...
  for(const [id,value,offset] of [['text',T('\ud800'),1],['key',O([['\udfff',Null()]]),2],['number',N('\ud800'),0]])yield {id:`encode/scalar/${id}`,value,limits:limits(),nativeScalar:true,expect:{kind:'fail',code:'EInvalidScalar',offset}};
  ...
  for(const n of [1,2,3]){
    ...
    let nested=Null();for(let i=0;i<n;i++)nested=A([nested]);yield {id:`encode/boundary/depth/${n}`,value:nested,limits:limits({max_depth:2n}),expect:n<=2?{kind:'done',text:encodeExpected(nested)}:{kind:'fail',code:'EDepthLimit',offset:2}};
  }
  ...
  const values=[Null(),T(''),T('\0'),T('\b\f\n\r\t'),T('"\\/'),A([Null(),T('😀')]),O([['x',N('-0')],['x',T('é')]])];
  for(let i=0;i<values.length;i++){const value=values[i],text=encodeExpected(value),n=codepoints(text);for(const capacity of [n-1,n,n+1])yield {id:`encode/output/${i}/${capacity}`,value,limits:limits({max_output:BigInt(capacity)}),expect:capacity<n?{kind:'fail',code:'EOutputLimit',offset:capacity}:{kind:'done',text}};}
  for(const [id,value,overrides,code,offset] of [['zero-output',Null(),{max_output:0n},'EOutputLimit',0],['zero-values',Null(),{max_values:0n},'EValueLimit',0],['zero-depth',A([]),{max_depth:0n},'EDepthLimit',0],['zero-number',N('0'),{max_number:0n},'ENumberLimit',0],['zero-string',T('a'),{max_string:0n},'EStringLimit',1],['number',N('123'),{max_number:2n},'ENumberLimit',0],['string',T('abc'),{max_string:2n},'EStringLimit',3],['key',O([['abc',Null()]]),{max_string:2n},'EStringLimit',4],['values',A([Null(),Null()]),{max_values:2n},'EValueLimit',6],['depth',A([A([])]),{max_depth:1n},'EDepthLimit',1],['output-precedence',N('bad'),{max_output:0n},'EOutputLimit',0]])yield {id:`encode/limits/${id}`,value,limits:limits(overrides),expect:{kind:'fail',code,offset}};
  for(const [id,value,overrides] of [['zero-input',Null(),{max_input:0n}],['zero-depth',N('0'),{max_depth:0n}],['zero-string',T(''),{max_string:0n}],['key-no-value-count',O([['k',Null()]]),{max_values:2n}],['astral',T('😀'),{max_string:1n,max_output:3n}]])yield {id:`encode/limits/pass/${id}`,value,limits:limits(overrides),expect:{kind:'done',text:encodeExpected(value)}};
  for(const field of Object.keys(limits()).filter(k=>k!=='$'))yield {id:`encode/invalid-limit/${field}`,value:Null(),limits:limits({[field]:16777217n}),expect:{kind:'fail',code:'EInvalidLimits',offset:0}};
  for(const field of Object.keys(limits()).filter(k=>k!=='$'))yield {id:`encode/cap/${field}`,value:Null(),limits:limits({[field]:16777216n}),expect:{kind:'done',text:'null'}};
}
export function* cases() {
  yield {id:'api/supported-defs-defaults',run(Core){for(const key of ['Json.parse','Json.encode','Json.number','Json.default_limits'])assert.equal(typeof Core[key],'function');assert.deepEqual(Core['Json.default_limits'](),limits());}};
  ...
  for(const item of numberCases())yield {id:item.id,run(Core){const result=Core['Json.number'](item.text,item.limits);if(item.expect.kind==='fail')failure(result,item.expect.code,item.expect.offset);else assertAst(done(result),item.expect.value);}};
  for(const item of encoderCases())yield {id:item.id,run(Core){const result=Core['Json.encode'](item.value,item.limits);if(item.expect.kind==='fail')failure(result,item.expect.code,item.expect.offset);else assert.equal(done(result),item.expect.text);}};
}

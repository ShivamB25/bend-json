...
The later per-call ABI validator (`4160adb`) is still open: locally it passed typecheck, proofs, both self-tests and 6,073 host cases per runtime (`2026-09-22T19:48:33.447Z`–`19:54:11.584Z`), but the native gate failed when a `construction-4.bend` compile hit its 120 s deadline under swap exhaustion. See [STATUS.md](STATUS.md) for the diagnosis.
